...
function ListHtml($classid,$fields,$enews=0,$userlistr=""){
	...
	if($enews==0)//子栏目列表
	{
		...
		$totalquery="select count(*) as total from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where ".$yhadd."classid='$classid' and checked=1";//统计
	...
	elseif($enews==1)//专题列表
	{
		...
		$totalquery="select count(*) as total from {$dbtbpre}ecms_".$class_zr[$classid][tbname]." where ".$yhadd."ztid like '%|".$classid."|%' and checked=1";//统计
	...
	elseif($enews==3)//大栏目列表
	{
		...
		$totalquery="select count(*) as total from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where ".$yhadd."(".$whereclass.") and checked=1";//统计
		$doclasspath=ReturnSaveClassPath($classid,0);
		$dopath=ECMS_PATH.$doclasspath."/";
		if(empty($class_r[$classid][classurl]))
		{
...
function ReturnLeftLevel($groupid){
	...
	{return "";}
	$groupid=(int)$groupid;
	$r=$empire->fetch1("select * from {$dbtbpre}m_group where groupid='$groupid'");
	return $r;
}
...
function CheckLevel($userid,$username,$classid,$enews){
	global $empire,$dbtbpre;
	$userid=(int)$userid;
	$r=$empire->fetch1("select groupid,adminclass from {$dbtbpre}m_user where userid='$userid' limit 1");
	//操作信息
	if($enews=="news")
	{
		//操作所有栏目权限
		$gr=$empire->fetch1("select doall,doselfinfo,doaddinfo,doeditinfo,dodelinfo from {$dbtbpre}m_group where groupid='$r[groupid]'");
		if(empty($gr[doall]))
		{
	...
	{
		//用户组
		$gr=$empire->fetch1("select * from {$dbtbpre}m_group where groupid='$r[groupid]'");
		$enews="do".$enews;
		if(empty($gr[$enews]))
		{
...
function is_login($uid=0,$uname='',$urnd=''){
	...
	//db
	$adminr=$empire->fetch1("select userid,groupid,classid from {$dbtbpre}m_user where userid='$userid' and username='".$username."' and rnd='".$rnd."' and checked=0 limit 1");
	if(!$adminr['userid'])
	{
...
function DoECreatFileRnd($userid,$username,$rnd,$dbdata,$groupid,$adminstyle,$truelogintime,$ip){
	...
	$file=ECMS_PATH.'\\data\\adminlogin\\user'.$userid.'_'.md5(md5($username.'-empirecms!check.file'.$truelogintime.'-'.$rnd.$do_ecookiernd).'-'.$ip.'-'.$userid.'-'.$rnd.$adminstyle.'-'.$groupid.'-'.$dbdata).'.log';
...
function DoEDelFileRnd($userid){
	$path=ECMS_PATH.'\\data\\adminlogin\\';
	$hand=@opendir($path);
...
function insert_dolog($doing){
	...
	$sql=$empire->query("insert into {$dbtbpre}m_dolog(username,logip,logtime,enews,doing) values('$loginin','$logip','$logtime','$enews','$doing');");
}
...
function GetTitleTypeCache(){
	global $empire,$dbtbpre;
	$sql=$empire->query("select typeid,tname,mid,yhid from {$dbtbpre}enewsinfotype order by typeid");
	while($r=$empire->fetch($sql))
	{
