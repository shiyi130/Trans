...
function DelLog($loginid,$userid,$username){
	...
	$sql=$empire->query("delete from {$dbtbpre}m_log where loginid='$loginid'");
	if($sql)
	{
		//操作日志
		insert_dolog("loginid=".$loginid);
		printerror("DelLogSuccess","InfoList.php");
	}
	else
	{
...
function DelLog_all($loginid,$userid,$username){
	...
	$sql=$empire->query("delete from {$dbtbpre}m_log where".$add);
	if($sql)
	{
		//操作日志
		insert_dolog("");
...
function DelLog_date($add,$userid,$username){
	...
	$endday=$end.' 23:59:59';
	$sql=$empire->query("delete from {$dbtbpre}m_log where logintime<='$endday' and logintime>='$startday'");
	if($sql)
	{
		//操作日志
...
$query="select loginid,username,loginip,logintime,status,password,loginauth from {$dbtbpre}m_log ";
$totalquery="select count(*) as total from {$dbtbpre}m_log";
//搜索
$search='';
$where='';
if($_GET['sear']==1)
...
<form name="form2" method="post" action="InfoList.php" onsubmit="return confirm('确认要删除?');">
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="tableborder">
    <tr class="header">
      <td width="7%"><div align="center"><a href="InfoList.php?orderby=0&mydesc=<?=$loginiddesc.$search2?>">ID</a></div></td>
      <td width="28%" height="25"><div align="center"><a href="InfoList.php?orderby=1&mydesc=<?=$usernamedesc.$search2?>">登陆用户</a></div></td>
      <td width="20%"><div align="center"><a href="InfoList.php?orderby=2&mydesc=<?=$statusdesc.$search2?>">状态</a></div></td>
      <td width="17%" height="25"><div align="center"><a href="InfoList.php?orderby=3&mydesc=<?=$loginipdesc.$search2?>">登陆IP</a></div></td>
      <td width="17%"><div align="center"><a href="InfoList.php?orderby=4&mydesc=<?=$logintimedesc.$search2?>">登陆时间</a></div></td>
  ...
  ?>
    <tr bgcolor="#FFFFFF" id=log<?=$r[loginid]?>>
      ...
      <td height="25"><div align="center">[<a href=InfoList.php?enews=DelLog&loginid=<?=$r[loginid]?> onclick="return confirm('确认要删除此日志?');">删除</a> 
...
<form action="InfoList.php" method="post" name="dellogform" id="dellogform" onsubmit="return confirm('确认要删除?');">
